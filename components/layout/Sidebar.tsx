...
import { NAV_GROUPS, isLiveOnlyPath } from "@/lib/nav";
import { BRAND } from "@/lib/brand";
import { cn } from "@/lib/utils";
import { useIsOffline, useIsDemo } from "@/lib/hooks/use-offline";
...
export function Sidebar({ className }: { className?: string }) {
  ...
  // In a real uploaded-file session, hide views that need a live connection
  // (cost + telemetry). The demo synthesises that data, so it shows everything.
  const groups =
    isOffline && !isDemo
      ? NAV_GROUPS.map((g) => ({ ...g, items: g.items.filter((i) => !isLiveOnlyPath(i.href)) })).filter((g) => g.items.length > 0)
      : NAV_GROUPS;
